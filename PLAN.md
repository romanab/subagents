...
agents ALL=(root) NOPASSWD: /home/agents/bin/create-subaccount, \
  ...
  /home/agents/bin/bwrap-config-apply, /home/agents/bin/bwrap-config-reset, \
...
| `bwrap-config-apply --user NAME` | Reads `.subagent/config`, regenerates `.subagent/launcher` from scratch. Useful after manually editing the config or re-copying a profile. |
| `bwrap-config-reset --user NAME [--profile NAME]` | Replaces `.subagent/config` with a fresh copy of a profile (default: `default`), discarding any per-account customisations, then regenerates the launcher via `bwrap-config-apply`. The generated launcher accepts an optional command via `$@`; when called with no args it starts `/bin/bash` (normal session); when called with args it passes them to bwrap as the init command (used by `exec-in --sandbox`). |
